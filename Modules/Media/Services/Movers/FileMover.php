...
final class FileMover implements MoverInterface
{
    ...
    private function getFilenameFor(string $path, $thumbnail)
    {
        ...
        if ($filename === false) {
            return config('ceo.media.config.files-path') . $this->newFilename($path, $thumbnail);
        }

        return config('ceo.media.config.files-path') . $folders . $this->newFilename($path, $thumbnail);
    }
    ...
    {
        $configAssetPath = config('ceo.media.config.files-path');
