...
class MaxFolderSizeRule implements Rule
{
    ...
    {
        $mediaPath = public_path(config('ceo.media.config.files-path'));
        $folderSize = $this->getDirSize($mediaPath);

        preg_match('/([0-9]+)/', $folderSize, $match);

        return ($match[0] + $value->getSize()) < config('ceo.media.config.max-total-size');
    }
    ...
    {
        $bytes = config('ceo.media.config.max-total-size');
