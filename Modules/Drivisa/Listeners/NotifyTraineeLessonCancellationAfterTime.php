...
class NotifyTraineeLessonCancellationAfterTime implements ShouldQueue
{
    /**
    ...
    public function sendViaPostmark($lesson)
    {
        $this->mail->to($lesson->trainee->user->email)
            ->send((new \Coconuts\Mail\PostmarkTemplateMailable())
                    ->identifier(config('template.postmark.instructor_cancel_lesson_after_time_trainee_notify'))
                    ->include([
